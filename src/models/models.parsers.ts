...
let collectControlFlowInfo = (
  ...
  if (nonBlockingBoundaryEvents.length > 0) {
    ...
    // Let us remove all source elements from the node list
    mainPathNodeList = mainPathNodeList.filter(
      (node: string) => sources.indexOf(node) < 0
    );
    ...
    for (let eventId of nonBlockingBoundaryEvents) {
      ...
      // Let us remove all source elements from the node list
      localNodeList = localNodeList.filter(
        (node: string) => sources.indexOf(node) < 0
      );
  ...
  for (let subprocess of proc.flowElements.filter(e =>
    is(e, "bpmn:SubProcess")
  )) {
    let subprocessControlFlowInfo = collectControlFlowInfo(
      subprocess,
      globalNodeMap,
    ...
    if (
      ...
      controlFlowInfo.nodeList = controlFlowInfo.nodeList.concat(
        subprocessControlFlowInfo.nodeList
      );
      controlFlowInfo.edgeList = controlFlowInfo.edgeList.concat(
        subprocessControlFlowInfo.edgeList
      );
    }
  }
  ...
  new Promise((resolve, reject) => {
    parseBpmn(modelInfo.bpmn)
      .then((definitions: any) => {
        ...
        let mainControlFlowInfo = collectControlFlowInfo(
          proc,
          globalNodeMap,
          globalControlFlowInfo
        );
        ...
        for (let controlFlowInfo of globalControlFlowInfo) {
          ...
          indexesToRemove.sort((ind1, ind2) => ind2 - ind1);
          indexesToRemove.forEach(index => {
            controlFlowInfo.sources.splice(index, 1);
          });
          if (
            is(globalNodeMap.get(controlFlowInfo.self.id), "bpmn:SubProcess") &&
        ...
        for (let controlFlowInfo of globalControlFlowInfo) {
          ...
          if (!controlFlowInfo.isEmbedded) {
            ...
            controlFlowInfo.nodeList
              ...
              .forEach(e => {
                if (
                  (is(e, "bpmn:Task") || is(e, "bpmn:SubProcess")) &&
                  e.loopCharacteristics &&
                  e.loopCharacteristics.$type ===
                    "bpmn:MultiInstanceLoopCharacteristics"
                ) {
                  controlFlowInfo.multiinstanceActivities.set(
                    e.id,
                    modelInfo.name + ":" + getNodeName(e) + "_Contract"
                  );
                  multiinstanceActivities.push(e.id);
                  if (is(e, "bpmn:SubProcess"))
                    controlFlowInfo.childSubprocesses.set(
                      e.id,
                      modelInfo.name + ":" + getNodeName(e) + "_Contract"
                    );
                  ...
                  is(e, "bpmn:IntermediateCatchEvent") &&
            ...
            controlFlowInfo.boundaryEvents.forEach(nodeId => {
              ...
              if (!node.cancelActivity) {
                controlFlowInfo.nonInterruptingEvents.set(
                  node.id,
                  modelInfo.name + ":" + getNodeName(node) + "_Contract"
                );
            ...
            globalNodeMap.forEach(node => {
              if (
                ...
                for (let start of node.flowElements.filter(
                  ...
                  if (start.isInterrupting == false) {
                    ...
                    controlFlowInfo.nonInterruptingEvents.set(
                      start.id,
                      modelInfo.name + ":" + getNodeName(parent) + "_Contract"
                    );
            ...
            controlFlowInfo.nodeList.forEach(
              (nodeId: string, index: number, array: string[]) => {
                var node = globalNodeMap.get(nodeId);
                controlFlowInfo.nodeIndexMap.set(nodeId, index);
                globalNodeIndexMap.set(nodeId, index);
                controlFlowInfo.nodeNameMap.set(
                  nodeId,
                  getNodeName(globalNodeMap.get(nodeId))
                );
                if (
                  node.documentation &&
            ...
            controlFlowInfo.edgeList.forEach(
              (edgeId: string, index: number, array: string[]) => {
            ...
            let codeGenerationInfo = {
                ...
                controlFlowInfo.nodeList.forEach(nodeId => {
                  if (is(globalNodeMap.get(nodeId), "bpmn:SubProcess")) {
                    ...
                    subP.sources.forEach(id => {
                      if (
                        !is(globalNodeMap.get(id), "bpmn:Subprocess") &&
                  ...
                  if (node.eventDefinitions && node.eventDefinitions[0]) {
                    ...
                    if (cad.substring(5, cad.length - 15) === evType) {
                      if (is(node, "bpmn:StartEvent")) {
                        ...
                        if (node.$parent.id === subprocId) res.push(node.id);
                      }
                    }
                  }
                });
              ...
              getContracts2Call: () => {
                ...
                nonInterruptingEvents.forEach(evtId => {
                  ...
                  res.push(
                    is(node, "bpmn:StartEvent") ? node.$parent.id : evtId
                  );
                });
                return res;
              },
              ...
              isStartingContractEvent: (eventId, processId) => {
                ...
                if (is(evt, "bpmn:StartEvent")) {
                  ...
                  is(evt, "bpmn:IntermediateCatchEvent") &&
                  is(evt.eventDefinitions[0], "bpmn:MessageEventDefinition")
                )
                  return true;
                return false;
                ...
                globalNodeMap.forEach(node => {
                  if (node.$parent && node.$parent.id === subprocessId) {
                    if (is(node, "bpmn:Task"))
                      ...
                      !globalNodeMap.get(subprocessId).triggeredByEvent &&
                      node.eventDefinitions &&
                      node.eventDefinitions[0] &&
                      is(
                        node.eventDefinitions[0],
                  ...
                  for (let node of toSearch.flowElements.filter(
                    ...
                    if (node.$parent.id === subprocessId)
                      if (
                        ...
                        for (let outgoing of node.outgoing)
                          bitarray[
                            controlFlowInfo.edgeIndexMap.get(outgoing.id)
                          ] = 1;
                  }
                }
                for (let i = bitarray.length - 1; i >= 0; i--)
              ...
              subprocessMarking: subprocessId => {
                ...
                if (
                  is(
                    globalNodeMap.get(controlFlowInfo.self.id),
                    "bpmn:BoundaryEvent"
                  ) &&
                  multiinstanceActivities.indexOf(subprocessId) < 0
                ) {
                  var parentInfo = globalControlFlowInfoMap.get(
                    controlFlowInfo.self.$parent.id
                  );
              ...
              extendedSubprocessMarking: (subprocessId, orCondition) => {
                var candidates = callActivities.concat(
                  multiinstanceActivities.concat(nonInterruptingEvents)
                );
                var res = orCondition ? "|| (" : "\u0026" + "\u0026" + "(";
            ...
            controlFlowInfo.nodeList.forEach(nodeId => {
              ...
              if (
                ...
                if (
                  controlFlowInfo.localParameters.has(nodeId) &&
                  controlFlowInfo.localParameters.get(nodeId).length > 0
                ) {
                  userTaskList.push(nodeId);
                  parameterInfo.set(
                    nodeId,
                    controlFlowInfo.localParameters.get(nodeId)
                  );
            ...
            let workListGenerationInfo = {
              ...
              processId: () => controlFlowInfo.self.id,
              nodeName: nodeId => {
                if (nodeId === "defaultId") return "DefaultTask";
            ...
            modelInfo.controlFlowInfoMap.set(
              getNodeName(controlFlowInfo.self),
              controlFlowInfo
            );
          } else {
            controlFlowInfo.nodeList.forEach(nodeId =>
              controlFlowInfo.nodeIndexMap.set(
                nodeId,
                globalNodeIndexMap.get(nodeId)
              )
            );
            controlFlowInfo.edgeList.forEach(edgeId =>
              controlFlowInfo.edgeIndexMap.set(
                edgeId,
                globalEdgeIndexMap.get(edgeId)
              )
            );
          }
        }
